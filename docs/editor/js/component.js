 ...
 */
var Trigger = {
    BLOCK_BREAK          : { name: '破坏方块触发',          container: true, construct: TriggerBlockBreak         },
    BLOCK_PLACE          : { name: '放置方块触发',          container: true, construct: TriggerBlockPlace          },
    ...
    ENVIRONMENT_DAMAGE   : { name: '受到环境伤害触发',   container: true, construct: TriggerEnvironmentDamage  },
    ...
    MOVE                 : { name: '移动触发',                 container: true, construct: TriggerMove                },
...
var Condition = {
    ...
    CEILING:     { name: '顶棚',     container: true, construct: ConditionCeiling  },
    ...
    ELSE:        { name: '否则',        container: true, construct: ConditionElse  },
...
var Mechanic = {
    ...
    DURABILITY:          { name: '降低耐久',          container: false, construct: MechanicDurability        },
    ...
    HEALTH_SET:          { name: '生命设置',          container: false, construct: MechanicHealthSet,         },
    HELD_ITEM:           { name: '手持物品',           container: false, construct: MechanicHeldItem,          },
    ...
    CANCEL_EFFECT:       { name: '取消粒子效果',       container: false, construct: MechanicCancelEffect,       },
    ...
    TRIGGER:             { name: '监听',             container: true,  construct: MechanicTrigger,            },
    ...
    VALUE_COPY:          { name: '复制变量值',          container: false, construct: MechanicValueCopy          },
    VALUE_DISTANCE:      { name: '获取距离值',      container: false, construct: MechanicValueDistance      },
    VALUE_HEALTH:        { name: '获取生命值',        container: false, construct: MechanicValueHealth       },
    ...
    VALUE_LORE_SLOT:     { name: '获取描述行值',     container: false, construct: MechanicValueLoreSlot     },
    VALUE_MANA:          { name: '获取魔力值',          container: false, construct: MechanicValueMana         },
    VALUE_MULTIPLY:      { name: '获取倍率值',      container: false, construct: MechanicValueMultiply      },
    VALUE_PLACEHOLDER:   { name: '获取占位符值',   container: false, construct: MechanicValuePlaceholder
	},
...
function MechanicDamage()
{
    ...
    this.data.push(new StringValue('分类器', 'classifier', 'default')
        .setTooltip('  伤害类型[例如elemental damage或fake physical damage]')
    );
...
function MechanicSpeed()
{
    ...
    this.data.push(new AttributeValue('倍率', 'amount', 5, 2)
        .setTooltip('移速倍率 [加速BUFF同为基本值]')
    );
    this.data.push(new AttributeValue('持续时间', 'seconds', 3, 0)
        .setTooltip('持续时间 [秒]')
    );
